...
// returns all acces rights for a plot
    ...
    public void MarkBuildingsAsUnPlaced(int buildingToGiveBack, int PlotID) {
        ...
        try {
           ...
           // executeQuery.next();
            while (executeQuery.next()) {
                buildingid = Integer.parseInt(executeQuery.getString("BuildingID"));
            }
        ...
        sql = "UPDATE TOP(1) BuildLog SET BuildLogPlaced='0' WHERE BuildLogPlotID=" + PlotID + " AND BuildLogBuildingID=" + buildingid+" AND BuildLogPlaced='1'";
        System.out.println(sql);
        try {
            stmt = con.createStatement();
            stmt.execute(sql);
